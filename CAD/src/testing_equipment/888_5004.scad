...
module 888_5004() {
    ...
    translate([ALU_profile_width/2+ALU_profile_holder_wall_thickness-(ALU_profile_width+ALU_profile_holder_wall_thickness*2-(608_bearing_thickness*2+podlozka*2+4))/2, 0, 0])
        difference() {
            hull() {
                cube([ALU_profile_width+ALU_profile_holder_wall_thickness*2-(608_bearing_thickness*2+podlozka*2+4), 5, ALU_profile_width]);
                
                translate([0, 608_bearing_outer_diameter-5, ALU_profile_width/2])
                    rotate([0, 90, 0])
                        cylinder(d=M8_screw_diameter+10, h=ALU_profile_width+ALU_profile_holder_wall_thickness*2-(608_bearing_thickness*2+podlozka*2+4), $fn=60);
            }
